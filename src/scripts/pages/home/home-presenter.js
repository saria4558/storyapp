...
async saveReport(story) {
  try {
    // langsung simpan data yang sudah ada, tidak fetch ulang
    ...
    this.#view.saveToBookmarkFailed(error.message || 'Gagal menyimpan bookmark');
  }
}
...
async removeReport(story) {
  try {
    ...
    this.#view.removeFromBookmarkFailed(error.message || 'Gagal menghapus bookmark');
  }
}
